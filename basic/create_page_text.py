...
# ✅ Check arguments
if len(sys.argv) != 3:
    print("Usage: create_page_text.py <colors> <api_key>")
    sys.exit(1)

colors_arg = sys.argv[1]  # "main" or "secondary"
api_key = sys.argv[2]

where = lambda colors: 2 if colors == "main" else 1 if colors == "secondary" else None

if where(colors_arg) is None:
    print("Invalid colors argument. Must be 'main' or 'secondary'.")
    sys.exit(1)
...
with open("basic/pdf_text.json", "w", encoding="utf-8") as f:
